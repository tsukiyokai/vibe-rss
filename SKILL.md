...
```
sources.txt --> fetch_urls.py --> Claude筛选+摘要 --> weekly-YYYY-MM-DD.md
```
...
读取 `~/note/proj/vibe-rss/sources.txt`，解析:
- `>` 前缀行 → 兴趣标签列表
- `#` 行 → 分类(忽略)
- 裸URL → 待抓取源列表
...
```bash
python3 ~/.claude/skills/vibe-rss/scripts/fetch_urls.py ~/note/proj/vibe-rss/sources.txt
```
...
输出文件: `~/note/proj/vibe-rss/weekly-YYYY-MM-DD.md`
